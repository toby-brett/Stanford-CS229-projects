...
class MultiNormal 
{
    public static double PDF(double[] mu, double[,] covarMat, double[] X)
    {
        int dims = covarMat.Length;
        double frac = (1 / (Math.Pow((2 * Math.PI), (dims / 2)) * Math.Pow(Matrix.Determinant(covarMat), 2)));
        double[] XminusMu = Elementwise.Subtract(X, mu);
        double[,] XminusMuT = Matrix.Transpose(XminusMu);
        double[] XminusMuCovarMat = Matrix.Multiply(XminusMu, Matrix.Inverse(covarMat));
        double term2 = Math.Exp(-0.5d * Matrix.Dot(XminusMuCovarMat, XminusMu));
...
class Test_Train_Split
{
    ...
    public static (List<double[]>, List<double[]>, List<int>, List<int>) GetSplit(double percent_train)
    {
        List<string[]> Data = Datareader.ReadCsvFile("data.csv");
        ...
        List<double[]> X_train = X.GetRange(0,index); // [:100] python equiv
        List<double[]> X_test = X.GetRange(index, Data.Count - index);
    ...
    public static (double, double[], double[], double[,]) DefineParams(List<double[]> X_train, List<int> y_train)
    {
        double phi = 0D;
        double[] mu0 = new double[4]; // 4 features so a 4 dimensional mean
        double[] mu1 = new double[4];
        double[,] covarMat = new double[4,4];
        ...
        foreach (var example in zippedList)
        {
            if (example.y == 1)
            {
                ...
                {
                    mu1[i] += example.x[i]; // will divide later for now just sum
                }
            }
            ...
            if (example.y == 0)
            {
                ...
                for (int i = 0; i < example.x.Length; i++)
                {
                    mu0[i] += example.x[i]; // will divide later for now just sum
                }
            }
        }

        mu0 = Elementwise.Divide(mu0, count0s);
        mu1 = Elementwise.Divide(mu1, count1s);
        ...
        foreach (var example2 in zippedList)
        {
            if (example2.y == 1)
            {
                double[,] outerProd = Matrix.OuterProduct(example2.x.Subtract(mu1), example2.x.Subtract(mu1));
                covarMat = Elementwise.Add(covarMat, outerProd);
            }
            else
            {
                double[,] outerProd = Matrix.OuterProduct(example2.x.Subtract(mu0), example2.x.Subtract(mu0));
                covarMat = Elementwise.Add(covarMat, outerProd);
            }
        }

        covarMat = Elementwise.Divide(covarMat, X_train.Count);
...
class GDA
{
    private (List<double[]>, List<double[]>, List<int>, List<int>) _data;
    ...
    private int y_test;
    private double phi;
    private double[] mu0, mu1;
    private double[,] covarMat;
    ...
    public GDA()
    {
        ...
        var parameters = Test_Train_Split.DefineParams(X_train, y_train); // X_train y_train
    ...
    public int Test(int i) // index to test 
    {
        ...
        double[] X_test = _data.Item2[i]; // only test one index at a time
...
class Datareader
{
    public static List<string[]>
        ReadCsvFile(
            string filepath) // static - instance not needed, List<string[]> returns a list of string arrays where each array is a row of the csv
    {
        List<string[]>
            rows = new List<string[]>(); // creates empty list, with each row being an array of strings. List<data type of each entry>
...
class main
    ...
    static void Main() // main method, the entry point of the program
    {
        ...
        List<string[]> csvData = Datareader.ReadCsvFile(csvFilePath);
        GDA model = new GDA();
